...
// 🧮 HELPERS
// =======================================================
const clamp = (v, [min, max]) => v == null || isNaN(v) ? 0 : Math.min(Math.max(v, min), max);
const normalize = (v) => Math.round((Number(v || 0) + Number.EPSILON) * 100) / 100;
function fixUnits(v) {
    if (v > 900)
        return normalize(v / 10);
    if (v > 90)
        return normalize(v / 10);
    return normalize(v);
}
...
const scientificCorrection = async (req, res) => {
    ...
    try {
        ...
        for (const f of allFoods.rows) {
         ...
         WHERE id=$5`, [kcal, protein, fat, carbs, id]);
            // 6️⃣ Log correction
...
// =======================================================
exports.default = scientificCorrection;
