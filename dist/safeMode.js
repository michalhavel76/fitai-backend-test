...
exports.verifyDatabaseSafety = verifyDatabaseSafety;
...
// =======================================================
async function logBlockedAction(operation, reason) {
    try {
...
// =======================================================
async function assertSafe(operation) {
    ...
    if (env !== "dev" && env !== "development") {
        ...
        console.log("🛡️ FitAI SafeMode prevented potential data loss. Exiting process...");
        process.exit(0);
    }
}
// =======================================================
...
async function verifyDatabaseSafety() {
    try {
        const client = await pool.connect();
        const { rows } = await client.query(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public';`);
