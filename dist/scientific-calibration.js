...
function detectCategory(name) {
    ...
    if (/(chicken|beef|pork|turkey|ham|meat|steak)/.test(n))
        return "meat";
    if (/(fish|salmon|tuna|cod|trout|shrimp)/.test(n))
        return "fish";
    if (/(milk|yogurt|cheese|butter|cream)/.test(n))
        return "dairy";
    if (/(apple|banana|orange|berry|grape|fruit)/.test(n))
        return "fruit";
    if (/(potato|rice|pasta|flour|noodle|starch)/.test(n))
        return "starch";
    if (/(bread|cereal|baguette|toast|croissant|oat)/.test(n))
        return "bread/cereal";
    if (/(oil|fat|lard|butter|margarine)/.test(n))
        return "fat/oil";
    if (/(cake|cookie|sugar|sweet|dessert|chocolate)/.test(n))
        return "sweet";
    if (/(juice|cola|soda|water|tea|coffee|drink)/.test(n))
        return "drink";
    if (/(ketchup|mayo|sauce|dressing|mustard)/.test(n))
        return "sauce";
    if (/(vegetable|carrot|tomato|broccoli|onion|pepper|spinach)/.test(n))
        return "vegetable";
    return "unknown";
}
// =======================================================
...
const scientificCalibrate = async (req, res) => {
    ...
    try {
        ...
        for (let food of allFoods.rows) {
            ...
            const accuracyScore = Number(((kcalAcc + proteinAcc + fatAcc + carbsAcc) /
                (WEIGHT_MAP.kcal + WEIGHT_MAP.protein + WEIGHT_MAP.fat + WEIGHT_MAP.carbs)).toFixed(3));
            totalAccuracy += accuracyScore;
            calibrated++;
            if (accuracyScore < 0.8)
                outliers++;
            await client.query(`UPDATE foods SET category=$1, accuracy_score=$2, updated_at=NOW() WHERE id=$3`, [category, accuracyScore, food.id]);
            await client.query(`INSERT INTO food_audit_log (food_id, action, details, created_at)
         VALUES ($1,$2,$3,NOW())`, [
                food.id,
