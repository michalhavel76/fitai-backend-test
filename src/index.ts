...
app.post("/funny-message", upload.single("image"), async (req, res) => {
  try {
    ...
    if (!req.file)
      return res.json({ message: "Analyzuji tvoje jídlo... 😎" });
    ...
    const msg = funnyResp.choices?.[0]?.message?.content?.trim() || "Analyzuji tvoje jídlo... 😎";
    res.json({ message: msg });
  } catch (err) {
    console.error("Funny-message error:", (err as any).message);
    res.json({ message: "Analyzuji tvoje jídlo... 😎" });
  }
});
